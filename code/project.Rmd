...
```{r}
# 0 = dead, 1 = alive or died from other causes
data_melanoma[data_melanoma$status == 1,]$status <- 0
data_melanoma[data_melanoma$status == 2,]$status <- 1
data_melanoma[data_melanoma$status == 3,]$status <- 1
```
...
fit_bernoulli <- stan(file='bernoulli.stan', data=input_bernoulli, seed=SEED)
print(fit_bernoulli)
```

```{r}
#monitor(fit_bernoulli, probs = c(0.1, 0.5, .9))
draws <- as.data.frame(fit_bernoulli)
